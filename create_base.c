...
/*   Updated: 2017/02/06 16:36:49 by marnaud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

int			ft_option(t_point *point, t_win *param)
{
	if (param->color.oui == 0)
		return (0x00FFFFFF);
	if (point->z >= -2 && point->z <= 2)
		return (param->color.a);
	if ((point->z > 2 && point->z <= 4) || (point->z < -2 && point->z >= -4))
		return (param->color.b);
	if ((point->z > 4 && point->z <= 6) || (point->z < -4 && point->z >= -6))
		return (param->color.c);
	if ((point->z > 6 && point->z <= 8) || (point->z < -6 && point->z >= -8))
